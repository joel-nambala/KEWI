...
namespace KEWIVendor.Controllers
{
    public class PrequalificationController : Controller
    {
        ...
        public ActionResult PrequalificationApplication()
        {
            ...
            try
            {
                string vatNo = Session["VATNo"].ToString();
                string vendorEmail = Session["VendorEmail"].ToString();

                // Check if prequalification applied
                if (webportals.HasAppliedForPrequalification(vatNo, vendorEmail))
                {
        ...
        public ActionResult SubmitCategoryDocument(Prequalification prequalification)
        {
            ...
            try
            {
                ...
                string vendorVat = Session["VATNo"].ToString();
                string path = Server.MapPath("~/Attachments/");
                if (!webportals.PrequalificationApplied(vendorVat))
                {
                    webportals.CreateVendorPrequalificationHeader(vendorNo);
                }
                if(!Directory.Exists(path)) Directory.CreateDirectory(path);
                if (prequalification.AttachmentFile != null && prequalification.AttachmentFile.ContentLength > 0)
                {
                    string filename = vendorVat + prequalification.DocName;
        ...
        public ActionResult SubmitCategory(Prequalification prequalification)
        {
            ...
            try
            {
                ...
                webportals.CreatePrequalificationLine(vatNo, vendorEmail, category);
        ...
        public ActionResult SubmitApplication()
        {
            try
            {
                ...
                webportals.SubmitPrequalificationApplication(vatNo, vendorEmail);
